...
get_nvs_peer_record(char *key_string, struct ble_hs_dev_records *p_dev_rec)
{
    ...
    /*
     * A bond blob written by a different NimBLE version can have a
     * different structure size. Never let NVS copy a larger blob into
     * this fixed-size structure.
     */
    if (required_size != sizeof(struct ble_hs_dev_records)) {
        ESP_LOGW(LOG_TAG,
                 "Ignoring incompatible peer record: stored=%u expected=%u",
                 (unsigned)required_size,
                 (unsigned)sizeof(struct ble_hs_dev_records));
...
get_nvs_db_value(int obj_type, char *key_string, union ble_store_value *val)
{
    ...
    /*
     * Bond records can survive firmware changes. If an older/newer
     * NimBLE version used a different structure size, using the stored
     * blob length here would overflow the destination and smash the stack.
     */
    if (obj_type == BLE_STORE_OBJ_TYPE_CCCD) {
        if (required_size != sizeof(struct ble_store_value_cccd)) {
            ESP_LOGW(LOG_TAG,
                     "Ignoring incompatible CCCD record: stored=%u expected=%u",
                     (unsigned)required_size,
                     (unsigned)sizeof(struct ble_store_value_cccd));
            err = ESP_ERR_NVS_INVALID_LENGTH;
            goto end;
        }
        ...
        if (required_size != sizeof(struct ble_store_value_sec)) {
            ESP_LOGW(LOG_TAG,
                     "Ignoring incompatible security record: stored=%u expected=%u",
                     (unsigned)required_size,
                     (unsigned)sizeof(struct ble_store_value_sec));
            err = ESP_ERR_NVS_INVALID_LENGTH;
            goto end;
        }
...
/*
 * Chatter v2.1 / Meshtastic 2.7 migration:
 * Clear only the NimBLE bond namespace once, then retain all new 2.7 bonds.
 */
static void ble_nvs_migrate_incompatible_bonds_once(void)
{
    nvs_handle_t nimble_handle;
    esp_err_t err;
    ...
    err = nvs_open(NIMBLE_NVS_NAMESPACE, NVS_READWRITE, &nimble_handle);
    if (err != ESP_OK) {
        ESP_LOGE(LOG_TAG, "Unable to open NimBLE NVS namespace for migration");
        return;
    }

    err = nvs_get_u8(nimble_handle, "v27_clean", &migration_done);
    if (err == ESP_OK && migration_done == 1) {
        nvs_close(nimble_handle);
        return;
    }

    printf("BLE NVS MIGRATION: clearing incompatible legacy bond records once\n");
    fflush(stdout);

    err = nvs_erase_all(nimble_handle);
    if (err != ESP_OK) {
        ESP_LOGE(LOG_TAG, "Unable to erase legacy NimBLE bond records");
        nvs_close(nimble_handle);
        return;
    }

    err = nvs_set_u8(nimble_handle, "v27_clean", 1);
    if (err != ESP_OK) {
        ESP_LOGE(LOG_TAG, "Unable to write NimBLE migration marker");
        nvs_close(nimble_handle);
        return;
    }
    ...
    err = nvs_commit(nimble_handle);
    if (err != ESP_OK) {
        ESP_LOGE(LOG_TAG, "Unable to commit NimBLE bond migration");
        nvs_close(nimble_handle);
        return;
    }

    printf("BLE NVS MIGRATION: legacy bond records cleared; future bonds retained\n");
    fflush(stdout);
    nvs_close(nimble_handle);
}
...
void ble_store_config_conf_init(void)
{
    ...
    ble_nvs_migrate_incompatible_bonds_once();
